...
MeshEdge *MeshEdge::nextAroundEndpoint(int i)
{
...
void MeshEdge::updateQuadricCollapseError()
{
    // Update both quadric_collapse_error and quadric_collapse_position, using
    // the existing endpoint quadrics and the method of
    // Garland/Heckbert.
    //
    // NOTE: Remember to check if the quadric Q' is invertible. If not, you will
    // have to use a fallback option such as the
    // midpoint of the edge (or in the worst case, set the error to a negative
    // value to indicate this edge should not be
    // collapsed).

    DMat4 q = endpoints[0]->getQuadric() + endpoints[1]->getQuadric();

    DMat4 w(q(0, 0), q(0, 1), q(0, 2), q(0, 3), q(0, 1), q(1, 1), q(1, 2),
            q(1, 3), q(0, 2), q(1, 2), q(2, 2), q(2, 3), 0, 0, 0, 1);
    ...
    if (w.determinant() > 1e-3) {
        ...
        v = Vector4(
            endpoints[0]->getPosition() + endpoints[1]->getPosition() / 2.0, 1);
    }
