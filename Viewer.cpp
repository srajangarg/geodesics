...
bool initMeshShader(Graphics::Shader &shader)
{
    ...
    static std::string const FRAGMENT_SHADER
        = "uniform vec3 ambient_color;\n"
          "uniform vec3 light_dir;  // must be specified in camera space, "
          "pointing towards object\n"
          ...
          "\n"
          "  float NdL = -dot(N, L);\n"
          "  vec3 lamb_color = (NdL >= -two_sided) ? material[1] * abs(NdL) * "
          "gl_Color.rgb * light_color : vec3(0.0, 0.0, 0.0);\n"
          "\n"
          "  gl_FragColor = vec4(ambt_color + lamb_color, gl_Color.a);\n"
          "}\n";

    try {
        shader.attachModuleFromString(Graphics::Shader::ModuleType::VERTEX,
                                      VERTEX_SHADER.c_str());
...
void Viewer::draw()
{
    ...
    if (mesh) {
        ...
        static Graphics::Shader *mesh_shader
            = NULL; // Ok since Viewer fns are all static. Else should have a
                    // shader per Viewer.
        if (!mesh_shader) {
        ...
        mesh->draw(*render_system, /* draw_edges = */ show_edges,
                   /* use_vertex_data = */ false, /* send_colors = */ false);
...
Matrix3 dragToRotation(int x1, int y1, int x2, int y2, int width, int height,
                       Camera const &camera)
{
    static Real const ROT_SPEED = 5;

    // Pixel coordinates increase top to bottom, so dy is downwards in camera
    // space
    int dx = x2 - x1;
    int dy = y2 - y1;
    if (dx == 0 && dy == 0)
...
Real dragToScale(int x1, int y1, int x2, int y2, int width, int height,
                 Camera const &camera)
{
...
void Viewer::keyPress(unsigned char key, int x, int y)
{
    if (key == 27) {
        exit(0);
    } else if (key == 'b' || key == 'B') {
...
void Viewer::incrementViewTransform(AffineTransform3 const &tr)
{
    AffineTransform3 inv_vt = tr.inverse();
    CoordinateFrame3 const &old_cframe = camera.getFrame();
    camera.setFrame(CoordinateFrame3(RigidTransform3::_fromAffine(
        AffineTransform3(inv_vt.getLinear() * old_cframe.getRotation(),
                         inv_vt * old_cframe.getTranslation()))));

    glutPostRedisplay();
}

void Viewer::mousePress(int button, int state, int x, int y)
{
...
void Viewer::mouseMotion(int x, int y)
{
    ...
    if (modifier_keys == GLUT_ACTIVE_SHIFT) {
        ...
        Vector3 dir;
        if (zoom > 1) {
            Vector2 drag_pos(2 * drag_start_x / (Real)width - 1,
                             1 - 2 * drag_start_y / (Real)height);
...
void Viewer::drawOutlineBox(AxisAlignedBox3 const &bbox)
{
    if (!render_system)
        return;
