...
void Mesh::remove_from_heap(std::multiset<Edge *, EdgeComp> &eh, Edge *e)
{
...
MeshEdge *Mesh::mergeEdges(Edge *e0, Edge *e1)
{
    if (!e0)
        return e1;
    if (!e1)
    ...
    alwaysAssertM(e0->isCoincidentTo(*e1),
                  std::string(getName())
                      + ": Edges to merge must have the same endpoints");
    ...
    for (Edge::FaceIterator fi = e1->facesBegin(); fi != e1->facesEnd(); ++fi) {
        ...
        (*fi)->replaceEdge(e1, e0); // in case the face references both e1 and
                                    // e0, we need to do the replacement even if
                                    // the above
    ...
    if (u->numFaces() <= 0 && u->numEdges() <= 0)
        vertices_to_remove[0] = u;
    if (v->numFaces() <= 0 && v->numEdges() <= 0)
        vertices_to_remove[1] = v;
...
MeshVertex *Mesh::collapseEdge(Edge *edge)
{
    if (!edge)
        return NULL;
    ...
    // Check if u is a repeated vertex in any face. If so, preferentially remove
    // it (one copy at a time)
    bool stop = false;
    for (FaceConstIterator fi = faces.begin(); fi != faces.end(); ++fi) {
        int num_occurrences = 0;
        for (MeshFace::VertexConstIterator vi = fi->verticesBegin();
             vi != fi->verticesEnd(); ++vi) {
            if (*vi == u) {
                num_occurrences++;
    ...
    for (size_t i = 0; i < edge_faces.size(); ++i) {
        ...
        // The edge and vertex have to be removed in linked fashion from the
        // loops around the face. This is tricky.
        while (true) {
            MeshFace::VertexIterator vi = face->verticesBegin();
            bool found = false;
            for (MeshFace::EdgeIterator ei = face->edgesBegin();
                 ei != face->edgesEnd(); ++ei, ++vi) {
                if (*ei != edge)
                    continue;
                ...
                if (*vi == v)
                    ...
                    ++vi;
                    bool last = false;
                    if (vi == face->verticesEnd()) {
                        vi = face->verticesBegin();
                        last = true;
                    }
                    if (*vi == v) {
                        face->vertices.erase(vi);
                        ...
                        DGP_ERROR << getName()
                                  << ": Vertex not found in expected location";
                        return NULL;
                    }
                }
    ...
    // Faces adjacent to the edge do not reference it, or v, any more. 'edge' is
    // isolated except for its references to u and v.
    // The mesh is in an inconsistent state since the face does not reference v
    // whereas the next edge around the face does.

    for (Vertex::EdgeIterator vei = v->edgesBegin(); vei != v->edgesEnd();
         ++vei) {
    ...
    // No edges reference v any more. The mesh is in an inconsistent state since
    // faces incident to v but not containing 'edge' do
    // not contain u yet.

    for (Vertex::FaceIterator vfi = v->facesBegin(); vfi != v->facesEnd();
         ++vfi) {
        (*vfi)->replaceVertex(
            v, u); // no-op if we had removed v from this face above
    ...
    while (found) {
        ...
        for (Vertex::EdgeIterator vei = u->edgesBegin(); vei != u->edgesEnd();
             ++vei) {
    ...
    // All double edges have been collapsed to single edges (this can happen
    // either because faces were shrunk to zero, or because
    // the mesh has genus > 0).

    return u;
}

MeshVertex *Mesh::decimateQuadricEdgeCollapse()
{
    DGP_CONSOLE << getName() << ": Mesh has " << numFaces()
                << " faces, collapsing a single edge";

    // The general scheme here is:
    // (1) Loop over edges to find the one with the minimum error (remember to
    // check if the error is negative, in which case the
    //     edge is invalid for collapsing).
    // (2) Collapse the min error edge, if a valid one is found (else return
    // NULL). There is a convenient function for this (it is
    //     a good exercise to study how this function operates). Make a note of
    //     the single vertex which is the result of the
    //     collapse.
    // (3) Update the normal of every face incident on this vertex.
    // (4) Update the quadric for this vertex, as well as the normal of the
    // vertex (there are convenient functions for this, one
    //     of which you will write yourself).
    // (5) For every edge incident on this vertex:
    //     - Update the quadric and normal for its other endpoint, which has
    //     also been affected.
    //     - Update the quadric collapse error and the optimal collapse position
    //     for the edge.
    // (6) Return the vertex.
...
void Mesh::decimateQuadricEdgeCollapse(long target_num_faces)
{
    ...
    while (numFaces() > target_num_faces) {
        ...
        // This might help to debug stuff. Remember that if you messed up the
        // mesh, this saving step can also crash.
...
bool Mesh::saveOFF(std::string const &path) const
{
    ...
    long index = 0;
    for (VertexConstIterator vi = vertices.begin(); vi != vertices.end();
         ++vi, ++index) {
    ...
    for (FaceConstIterator fi = faces.begin(); fi != faces.end(); ++fi) {
        ...
        for (Face::VertexConstIterator vi = fi->verticesBegin();
             vi != fi->verticesEnd(); ++vi) {
