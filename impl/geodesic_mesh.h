...
public:
    ...
    void initialize_mesh_data(
        unsigned num_vertices, Points &p, unsigned num_faces,
    ...
    unsigned closest_vertices(SurfacePoint *p,
                              ...
                              = NULL); // list vertices closest to the point
...
private:
    ...
    SimlpeMemoryAllocator<void_pointer> m_pointer_allocator; // fast memory
                                                             // allocating for
                                                             // Face/Vertex/Edge
                                                             // cross-references
};
...
template <class Points, class Faces>
void Mesh::initialize_mesh_data(
    Points &p,
    Faces &tri) // build mesh from regular point-triangle representation
{
    assert(p.size() % 3 == 0);
...
inline void Mesh::build_adjacencies()
{
    ...
    for (unsigned i = 1; i < half_edges.size(); ++i) {
        if (half_edges[i] != half_edges[i - 1]) {
            ...
            if (i < half_edges.size() - 1) // sanity check: there should be at
                                           // most two equal half-edges
...
Mesh::verify() // verifies connectivity of the mesh and prints some debug info
{
    ...
    // though the loose vertex is not a bug, it most likely indicates that
    ...
    std::cout << "approximate diameter of the mesh is "
              << sqrt(dx * dx + dy * dy + dz * dz) << std::endl;
