...
protected:
    ...
    node_pointer best_first_node(SurfacePoint &point, double &best_total_distance)
    {
        ...
        if (point.type() == VERTEX) {
            ...
            for (unsigned i = 0; i < possible_nodes.size(); ++i) {
                ...
                if (node->distance_from_source() + distance_from_dest
                    < best_total_distance) {
                    best_total_distance
                        = node->distance_from_source() + distance_from_dest;
                    best_node = node;
                }
            }
        }
    ...
    virtual void list_nodes_visible_from_node(
        ...
        std::vector<node_pointer> &storage, std::vector<double> &distances,
        double threshold_distance)
        = 0; // list only the nodes whose current distance is larger than the
             // threshold
...
template <class Node>
void GeodesicAlgorithmGraphBase<Node>::propagate(
        ...
        *stop_points) // or after ensuring that all the stop_points are covered
{
    ...
    std::vector<node_pointer>
        visible_nodes; // initialize vertices directly visible from sources
...
template <class Node>
inline void GeodesicAlgorithmGraphBase<Node>::trace_back(
...
template <class Node>
inline unsigned GeodesicAlgorithmGraphBase<Node>::best_source(
