...
public:
    ...
    GeodesicAlgorithmDijkstraAlternative(geodesic::Mesh *mesh = NULL)
        : GeodesicAlgorithmBase(mesh), m_nodes(mesh->vertices().size())
    {
        m_type = DIJKSTRA;
...
inline unsigned GeodesicAlgorithmDijkstraAlternative::best_source(
    ...
    double &best_source_distance)
{
    if (point.type() == VERTEX) {
        ...
        std::vector<vertex_pointer> possible_vertices; // initialize vertices
...
inline void GeodesicAlgorithmDijkstraAlternative::trace_back(
...
inline void GeodesicAlgorithmDijkstraAlternative::propagate(
    ...
    double max_propagation_distance, // propagation algorithm stops after
                                     ...
                                     // source
    std::vector<SurfacePoint>
        *stop_points) // or after ensuring that all the stop_points are covered
{
    ...
    std::vector<vertex_pointer>
        visible_vertices; // initialize vertices directly visible from sources
    ...
    while (!m_queue.empty()) // main cycle
    {
        if (counter++ % 10 == 0) // check if we covered all required vertices
        {
            ...
            // std::cout << counter << " " << m_queue.size() << " " <<
            // (*m_queue.begin())->distance_from_source()<< std::endl;
        }
        ...
        for (unsigned i = 0; i < v->adjacent_edges().size();
             ++i) // update all the adgecent vertices
        {
            ...
            if (next_node->distance_from_source()
                > min_node->distance_from_source() + e->length()) {
                if (next_node->distance_from_source()
...
GeodesicAlgorithmDijkstraAlternative::check_stop_conditions(unsigned &index)
{
    ...
    while (index < m_stop_vertices.size()) {
        ...
        if (queue_min_distance
            < node.distance_from_source() + m_stop_vertices[index].second) {
            return false;
        }
        ++index;
    }
