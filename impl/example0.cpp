/* read mesh from file and
 - if one vertex is specified, for all vertices of the mesh print their
 distances to this vertex
...
{
    if (argc < 2) {
        std::cout << "usage: mesh_file_name [first_vertex] [second_vertex]"
                  << std::endl; // try: "hedgehog_mesh.txt 3 14" or
                                // "flat_triangular_mesh.txt 1"
        return 0;
    }
    ...
    geodesic::GeodesicAlgorithmExact algorithm(
        &mesh); // create exact algorithm for the mesh
    ...
    if (argc > 3) // target vertex specified, compute single path
    {
        ...
        geodesic::SurfacePoint target(
            &mesh.vertices()[target_vertex_index]); // create source
        ...
        } else // doing the same thing explicitly for educational reasons
        {
            double const distance_limit
                = geodesic::GEODESIC_INF; // no limit for propagation
