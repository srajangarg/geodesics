...
private:
    ...
    unsigned intersect_intervals(interval_pointer zero,
                                 IntervalWithStop *one); // intersecting two
                                                         // intervals with up to
                                                         // three intervals in
                                                         // the end

    interval_pointer best_first_interval(SurfacePoint &point, double &best_total_distance,
...
inline double GeodesicAlgorithmExact::compute_positive_intersection(
    double start, double pseudo_x, double pseudo_y, double sin_alpha, double cos_alpha)
{
    assert(pseudo_y < 0);
        ...
        *one) // intersecting two intervals with up to three intervals in the end
{
    ...
    start[0] = zero->start(); // just to make sure that epsilons do not damage
                              // anything
    // start[N] = zero->stop();
...
inline void GeodesicAlgorithmExact::propagate(
    ...
    double max_propagation_distance, // propagation algorithm stops after
                                     // reaching the certain distance from the
                                     // source
    std::vector<SurfacePoint> *stop_points)
{
    ...
    while (!m_queue.empty()) {
             ...
             ++i) // two possible faces to propagate
        {
            ...
            if (num_propagated) {
                ...
                bool const invert
                    = next_edge->v0()->id() != edge->v0()->id(); // if the origins
                                                                 // coinside, do not
                                                                 // invert intervals
            ...
            if (propagate_to_right) {
                ...
                if (num_propagated) {
                    bool const invert
                        = next_edge->v0()->id() != edge->v1()->id(); // if the origins
                                                                     // coinside, do not
                                                                     // invert intervals
    ...
    IntervalWithStop *candidates) // if it is the last interval on the edge
{
    ...
    // if the new edge can be covered only after turn_over, the value is
    // negative (-1.0)
    double L1
        = compute_positive_intersection(begin, pseudo_x, pseudo_y, sin_alpha, cos_alpha);
    ...
    double L2
        = compute_positive_intersection(end, pseudo_x, pseudo_y, sin_alpha, cos_alpha);
...
inline interval_pointer GeodesicAlgorithmExact::best_first_interval(
    SurfacePoint &point, double &best_total_distance, double &best_interval_position,
    unsigned &best_source_index)
{
    assert(point.type() != UNDEFINED_POINT);
    ...
    std::vector<SurfacePoint> &path)
{
    ...
    if (best_interval) // if we did not hit the face source immediately
    {
        ...
        while (visible_from_source(path.back())
               < 0) // while this point is not in the direct visibility of some
                    // source (if we are inside the FACE, we obviously hit the
                    // source)
        {
            ...
            best_point_on_the_edge_set(q, possible_edges, interval, total_distance,
                                       position);
