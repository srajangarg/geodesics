...
public:
    ...
    SubdivisionNode(Pointer p)
        : SurfacePoint(p), m_previous(NULL), m_distance(0.0){};

    template <class Pointer, class Parameter>
    SubdivisionNode(Pointer p, Parameter param)
        : SurfacePoint(p, param), m_previous(NULL), m_distance(0.0){};
    ...
    {
        return static_cast<SurfacePoint &>(*this);
    };
...
protected:
    void list_nodes_visible_from_source(
        ...
        std::vector<node_pointer>
            &storage); // list all nodes that belong to this mesh element
    ...
    void list_nodes_visible_from_node(
        ...
        double threshold_distance); // list only the nodes whose current
...
void GeodesicAlgorithmSubdivision::list_nodes_visible_from_source(
    MeshElementBase *p, std::vector<node_pointer> &storage)
{
    assert(p->type() != UNDEFINED_POINT);
...
void GeodesicAlgorithmSubdivision::list_nodes_visible_from_node(
    node_pointer node, // list all nodes that belong to this mesh element
    std::vector<node_pointer> &storage, std::vector<double> &distances,
    double threshold_distance)
{
